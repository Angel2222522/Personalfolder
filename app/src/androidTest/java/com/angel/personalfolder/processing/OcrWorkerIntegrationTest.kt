...
class OcrWorkerIntegrationTest {
    ...
    fun workerReadsGreekImagePersistsTextAndUpdatesSearchIndex() = runBlocking {
        ...
        assertTrue("OCR text: ${document.ocrText}", foldedText.contains("ελληνικο"))
    ...
    fun workerRendersMultiPagePdfAndCombinesText() = runBlocking {
        val source = createPdf(
            listOf("ΕΛΛΗΝΙΚΟ ΚΕΙΜΕΝΟ"),
            listOf("ENGLISH TEXT 2026")
        )
        insertDocument(source, "application/pdf", "multi-page.pdf", pageCount = 2)
    ...
    @Test
    fun workerReportsBlankImageAsFailed() = runBlocking {
        val source = createTextImage()
        insertDocument(source, "image/png", "blank.png")
        ...
        assertNotNull(document)
        assertEquals(ProcessingState.FAILED, document!!.processingState)
        assertTrue(document.processingError.orEmpty().contains("χωρίς αναγνωρίσιμο κείμενο"))
    }

    private suspend fun runWorker(): ListenableWorker.Result = TestListenableWorkerBuilder<OcrWorker>(context)
        .setInputData(workDataOf(OcrWorker.KEY_DOCUMENT_ID to documentId))
        .build()
        .doWork()

    private suspend fun insertDocument(source: File, mimeType: String, name: String, pageCount: Int = 1) {
